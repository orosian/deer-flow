...
class TestChannelConnectionRepository:
    ...
    async def test_create_oauth_state_within_cap_enforces_pending_cap(self, repo):
        ...
        assert (
            await repo.create_oauth_state_within_cap(
                owner_user_id="alice", provider="slack", state="code-over", expires_at=expires, max_pending=3, now=now
            )
            is False
        )
        ...
        assert (
            await repo.create_oauth_state_within_cap(
                owner_user_id="bob", provider="slack", state="bob-1", expires_at=expires, max_pending=3, now=now
            )
            is True
        )
    ...
    async def test_create_oauth_state_within_cap_ignores_expired_rows(self, repo):
        ...
        for i in range(3):
            await repo.create_oauth_state(
                owner_user_id="alice", provider="slack", state=f"old-{i}", expires_at=now - timedelta(minutes=1)
            )
    ...
    async def test_create_oauth_state_within_cap_does_not_leak_under_concurrency(self, repo):
        ...
        async def issue(state: str) -> None:
            results.append(
                await repo.create_oauth_state_within_cap(
                    owner_user_id="alice", provider="slack", state=state, expires_at=expires, max_pending=3, now=now
                )
            )
