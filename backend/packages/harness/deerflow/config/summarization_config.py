...
class SummarizationConfig(BaseModel):
    ...
    model_name: str | None = Field(
        default=None,
        description=(
            "Model name to use for summarization (None = use a lightweight model)"
        ),
    )
    ...
    summary_prompt: str | None = Field(
        ...
        description=(
            ...
            "If not provided, uses the default LangChain prompt."
        ),
    )
    preserve_recent_skill_count: int = Field(
        ...
        description=(
            ...
            "Set to 0 to disable skill preservation."
        ),
    )
    preserve_recent_skill_tokens: int = Field(
        ...
        description=(
            ...
            "that must be preserved across summarization."
        ),
    )
    preserve_recent_skill_tokens_per_skill: int = Field(
        ...
        description=(
            ...
            "Skill reads above this size are not rescued."
        ),
    )
    skill_file_read_tool_names: list[str] = Field(
        default_factory=lambda: ["read_file", "read", "view", "cat"],
        description=(
            "Tool names treated as skill file reads when preserving "
            "recently-loaded skills across summarization."
        ),
    )
