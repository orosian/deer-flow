...
class SandboxConfig(BaseModel):
    ...
    AioSandboxProvider specific options:
        image: Docker image to use (default: enterprise-public-cn-beijing.cr.volces.com/vefaas-public/
            all-in-one-sandbox:latest)
        port: Base port for sandbox containers (default: 8080)
        replicas: Maximum number of concurrent sandbox containers (default: 3). When the limit is reached the
            least-recently-used sandbox is evicted to make room.
        container_prefix: Prefix for container names (default: deer-flow-sandbox)
        idle_timeout: Idle timeout in seconds before sandbox is released (default: 600 = 10 minutes). Set to 0 to
            disable.
        mounts: List of volume mounts to share directories with the container
        environment: Environment variables to inject into the container (values starting with $ are resolved from
            host env)
    """
    ...
    allow_host_bash: bool = Field(
        default=False,
        description=(
            "Allow the bash tool to execute directly on the host when using LocalSandboxProvider. "
    ...
    replicas: int | None = Field(
        default=None,
        description=(
            "Maximum number of concurrent sandbox containers (default: 3). "
            "When the limit is reached the least-recently-used sandbox is evicted to make room."
        ),
    )
    ...
    idle_timeout: int | None = Field(
        default=None,
        description=(
            "Idle timeout in seconds before sandbox is released (default: 600 = 10 minutes). "
            "Set to 0 to disable."
        ),
    )
    ...
    bash_output_max_chars: int = Field(
        ...
        description=(
            ...
            "Output exceeding this limit is middle-truncated (head + tail), preserving the first and last half. "
    ...
    read_file_output_max_chars: int = Field(
        ...
        description=(
            ...
            "Output exceeding this limit is head-truncated. "
            "Set to 0 to disable truncation."
        ),
    )
